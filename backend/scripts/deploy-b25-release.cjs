...
const allowed = ["public/index.html","public/script.js","public/profile-dashboard.js","public/profile-dashboard.css"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b25.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b25-profile-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B25 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B25 deployment failed:', error.code || error.message); process.exitCode = 1; });
