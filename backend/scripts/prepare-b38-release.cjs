...
const output = path.join(workspace, 'artifacts/ui-preview/b38-release');
const files = require('./b38-release-files.cjs');
...
const manifest = {
  ...
  release: 'b38-game-news-release-' + commit.slice(0, 10),
  ...
  files: files.map(name => ({
    ...
    baseline: cp.spawnSync('git', ['cat-file', '-e', baseline + ':' + name], { cwd: workspace }).status === 0
      ? contentHash(git(['show', baseline + ':' + name])) : null,
    sha256: contentHash(git(['show', commit + ':' + name]))
  }))
};
fs.writeFileSync(path.join(output, 'manifest.json'), JSON.stringify(manifest, null, 2));
for (const [source, target] of [
  ['deploy-b38-release.cjs', 'deploy.cjs'],
  ['rollback-b38-release.cjs', 'rollback.cjs'],
  ['b38-release-files.cjs', 'allowed.cjs']
