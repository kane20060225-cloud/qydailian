...
const allowed = ["public/index.html","public/community-pages.css"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b32.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b32-inline-nav-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B32 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B32 deployment failed:', error.code || error.message); process.exitCode = 1; });
