    ...
    for (const theme of ['dark', 'light']) for (const width of [1440, 390]) {
      ...
      try {
        ...
        const check = async name => {
          ...
          await page.screenshot({path: path.join(output, `live-b23-${theme}-${width}-${name}.png`), fullPage: true});
        };
        ...
        await page.evaluate(() => showSection('boost')); await page.locator('#boostNext').waitFor(); assert.equal(await page.locator('.credits-section h3').textContent(), 'Velnora 积分抵扣'); await check('boost');
