...
const allowed = ["public/index.html","public/script.js","public/theme-components.css","public/customer-support.css"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b29.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b29-mobile-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B29 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B29 deployment failed:', error.code || error.message); process.exitCode = 1; });
