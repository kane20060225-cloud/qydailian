...
const workspace=path.resolve(__dirname,'../..'),output=path.join(workspace,'artifacts/ui-preview/b29-release');
const files=["public/index.html","public/script.js","public/theme-components.css","public/customer-support.css"];
...
const manifest={commit,baseline_commit:baseline,release:'b29-mobile-release-'+commit.slice(0,10),archive_sha256:hash(fs.readFileSync(path.join(output,'runtime.tar'))),files:files.map(name=>({path:name,baseline:cp.spawnSync('git',['cat-file','-e',baseline+':'+name],{cwd:workspace}).status===0?contentHash(name,git(['show',baseline+':'+name])):null,sha256:contentHash(name,git(['show',commit+':'+name]))}))};
fs.writeFileSync(path.join(output,'manifest.json'),JSON.stringify(manifest,null,2));fs.copyFileSync(path.join(__dirname,'deploy-b29-release.cjs'),path.join(output,'deploy.cjs'));
