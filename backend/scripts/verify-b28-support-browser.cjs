...
(async()=>{
 const app=express();app.use(express.static(path.resolve(__dirname,'../../public')));const server=app.listen(0,'127.0.0.1');await new Promise(r=>server.once('listening',r));const origin=process.env.B28_ORIGIN||`http://127.0.0.1:${server.address().port}`,out=path.resolve(__dirname,'../../artifacts/b28-preview');fs.mkdirSync(out,{recursive:true});if(process.env.B28_ORIGIN)assert.equal(origin,'https://wotbqydailian.vip');let browser;
 ...
 for(const theme of ['dark','light'])for(const width of [1440,390,320]){
  ...
  await page.locator('#supportResolve').click();await page.waitForFunction(()=>document.getElementById('supportConversationStatus').textContent==='已解决');await page.screenshot({path:path.join(out,`${process.env.B28_ORIGIN?'live-':''}support-${theme}-${width}.png`)});
