...
const allowed = ['public/index.html', 'public/style.css', 'public/ui-foundation.css', 'public/esports-surfaces.css', 'public/bg.webp', 'public/images/tactical-grid.svg'];
const hash = bytes => crypto.createHash('sha256').update(bytes).digest('hex');
const contentHash = (name, bytes) => hash(name.endsWith('.webp') ? bytes : Buffer.from(bytes.toString('utf8').replace(/\r\n/g, '\n')));
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b22.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b22-esports-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    for (const file of manifest.files) {
      ...
      if (name.endsWith('.webp')) { assert.match(response.headers.get('content-type'), /image\/webp/); assert.equal(bytes.length, 65260); }
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B22 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B22 deployment failed:', error.code || error.message); process.exitCode = 1; });
