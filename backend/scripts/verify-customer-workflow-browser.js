...
(async()=>{
 ...
 try {
  ...
  for(const viewport of [{width:1440,height:1000},{width:390,height:844}]) {
   ...
   await page.evaluate(()=>showSection('admin'));await page.locator('#adminOrderList .oc-card').first().waitFor();assert.equal(sort,'priority');assert.match(await page.locator('#adminOrderList .oc-card').first().textContent(),/TEST-DISPUTE/);assert.match(await page.locator('#adminOrderList').textContent(),/超过提醒阈值/);await page.waitForFunction(()=>document.querySelector('[data-metrics]').textContent.includes('9 小时'));
