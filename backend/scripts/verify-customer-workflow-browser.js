...
(async()=>{
 ...
 try {
  ...
  for(const viewport of [{width:1440,height:1000},{width:390,height:844}]) {
   ...
   assert.match(await page.title(),/^QY Blitz \| WOT Blitz/);assert.equal(await page.locator('.logo-text').textContent(),'QY Blitz');assert.match(await page.locator('.credits-section h3').textContent(),/Velnora 积分/);
   ...
   assert.equal(await page.locator('#rentalFilterPanel').getAttribute('open'),null);assert.equal(await page.locator('#rentalFilters').isVisible(),false);await page.locator('#rentalFilterPanel > summary').click();assert.equal(await page.locator('#rentalFilters').isVisible(),true);
   ...
   await page.locator('#rentalFilterPanel > summary').click();assert.equal(await page.locator('#rentalFilters').isVisible(),false);assert.equal(await page.locator('.rental-account-card').count(),1);assert.equal(await page.locator('#rentalFilterResult').isVisible(),true);await page.locator('#rentalFilterPanel > summary').focus();await page.keyboard.press('Enter');assert.equal(await page.locator('#rentalFilters [name=tank]').inputValue(),'IS-7');
