  ...
  try {
    ...
    for(const viewport of [{width:1440,height:1000},{width:390,height:844}]) {
      ...
      const rows=()=>[
        ...
        {order_type:'shop',order_ref:'SHOP1',title:'测试兑换商品',customer_id:3,customer_name:'test-admin',amount:10,amount_unit:'credits',created_at:now,business_status:'completed',payment_status:'paid',payment_channel:'Velnora 积分',state:'completed',admin_task:null},
      ...
      await page.waitForFunction(()=>document.getElementById('orderCenterModal').getAttribute('aria-busy')==='false');await page.keyboard.press('Escape');await page.waitForFunction(()=>document.getElementById('orderCenterModal').style.display==='none');
      ...
      await page.waitForFunction(()=>document.getElementById('orderCenterModal').getAttribute('aria-busy')==='false');await page.keyboard.press('Escape');await page.locator('#orderCenterModal').waitFor({state:'hidden'});
      ...
      assert.ok((await page.locator('#adminOrderList .oc-trash-expiry').textContent()).includes('14天'));
      assert.equal((await page.locator('#adminOrderList .oc-trash-expiry').textContent()).includes('${'),false);
      assert.ok((await page.locator('#ocDetailBody').textContent()).includes('14天清理期限'));
      ...
      assert.ok((await page.locator('#ocDetailBody').textContent()).includes('回收站保留14天'));
