...
const allowed = ["public/index.html","public/script.js","public/theme-preference.js"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b34.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b34-theme-persistence-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B34 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B34 deployment failed:', error.code || error.message); process.exitCode = 1; });
