...
const allowed = ["public/index.html","public/script.js","public/community-pages.css"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b31.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b31-community-ui-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B31 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B31 deployment failed:', error.code || error.message); process.exitCode = 1; });
