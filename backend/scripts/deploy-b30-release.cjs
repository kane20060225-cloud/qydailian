...
const allowed = ["public/index.html","public/script.js"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b30.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b30-order-autofill-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B30 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B30 deployment failed:', error.code || error.message); process.exitCode = 1; });
