...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b36.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b36-nav-drag-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B36 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B36 deployment failed:', error.code || error.message); process.exitCode = 1; });
