...
const allowed = ["public/index.html"];
...
function atomic(name, bytes, mode) {
  ...
  const target = path.join(site, name), temporary = target + '.b35.tmp';
...
(async () => {
  ...
  assert.equal(root, '/root/b35-compact-intros-release-' + manifest.commit.slice(0, 10));
  ...
  try {
    ...
    if (changed && !verified) {
      ...
      assert.equal((await get('/api/health')).status, 200); console.error('B35 front-end rollback completed.');
    }
  }
})().catch(error => { console.error('B35 deployment failed:', error.code || error.message); process.exitCode = 1; });
