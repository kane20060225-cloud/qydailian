...
const {loadMigration,runMigration,SPECS}=require('../lib/b13-booster-availability-migration');
test('B13 plan is read only, apply is additive and detects schema conflicts',async()=>{
  ...
  const conn={execute:async(sql,p=[])=>{
    ...
    if(sql.includes('information_schema.statistics'))return [Object.entries(SPECS[p[0]].keys).flatMap(([name,columns])=>columns.map((column_name,i)=>({name,column_name,position:i+1,non_unique:name==='PRIMARY'||name.startsWith('uq_')?0:1})))];
  ...
  await runMigration(conn,{apply:true});assert.equal(ddl,2);assert.equal(checksum,loadMigration().checksum);assert.equal(locked,false);
  await runMigration(conn,{apply:true});assert.equal(tables.size,2);assert.equal(locked,false);
  conflict=true;await assert.rejects(()=>runMigration(conn,{apply:true}),/schema conflict/);assert.equal(locked,false);
});
test('B13 CLI rejects implicit connection and wrong confirmation',()=>{
  const cli=path.join(__dirname,'..','scripts','migrate-b13-booster-availability.js');
