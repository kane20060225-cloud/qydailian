...
test('rental admin tab exposes application review and keeps order review together', () => {
  ...
  assert.match(client, /\/admin\/rental\/accounts\?/);
  assert.match(client, /\/admin\/rental\/accounts\/\$\{checkedId\(id\)\}\/review/);
  assert.match(script, /rentalClient\.getAdminAccounts/);
});
