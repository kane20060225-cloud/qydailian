...
const {loadMigration,runMigration,SPECS}=require('../lib/b12-order-notifications-migration');
test('B12 plan is read only, apply is additive and detects schema conflicts',async()=>{
  ...
  await runMigration(conn,{apply:true});assert.equal(ddl,6);assert.equal(checksum,loadMigration().checksum);assert.equal(locked,false);
  await runMigration(conn,{apply:true});assert.equal(tables.size,6);assert.equal(locked,false);
  conflict=true;await assert.rejects(()=>runMigration(conn,{apply:true}),/schema conflict/);assert.equal(locked,false);
});
test('B12 CLI rejects implicit connection and wrong confirmation',()=>{
  const cli=path.join(__dirname,'..','scripts','migrate-b12-order-notifications.js');
