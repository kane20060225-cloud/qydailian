...
const {loadMigration,runMigration,SPECS}=require('../lib/b11-order-cleanup-migration');
test('B11 plan is read only, apply is additive and detects schema conflicts',async()=>{
...
test('B11 CLI rejects implicit connection and wrong confirmation',()=>{
  const cli=path.join(__dirname,'..','scripts','migrate-b11-order-cleanup.js');
