...
const VERSION = '20260916_b9_order_center';
const SPECS = {
  recharge_order_workflow: { columns: { out_trade_no:'varchar(64)',ticket_quantity:'int unsigned',provider_status:'varchar(30)',provider_checked_at:'datetime',credited_at:'datetime',credit_error_code:'varchar(60)' }, keys:{ PRIMARY:['out_trade_no'] } },
  order_management_state: { columns: { order_type:'varchar(20)',order_ref:'varchar(64)',archived_at:'datetime',archived_by:'int',archive_reason:'varchar(500)' },keys:{ PRIMARY:['order_type','order_ref'] } },
  order_management_events: { columns: { id:'bigint unsigned',order_type:'varchar(20)',order_ref:'varchar(64)',actor_user_id:'int',action:'varchar(50)',note:'varchar(500)' },keys:{ PRIMARY:['id'],idx_order_management_event:['order_type','order_ref','created_at','id'] } }
};
...
function loadMigration() {
  ...
  if (tables.join(',') !== Object.keys(SPECS).join(',')) throw new Error('B9 permits only the three reviewed additive tables');
...
async function readSchema(conn) {
  ...
  for (const [table,spec] of Object.entries(SPECS)) {
    ...
    for (const [name,type] of Object.entries(spec.columns)) {
      if (!columns.some((row)=>row.name===name && row.type.toLowerCase()===type)) throw new Error(`B9 schema conflict: ${table}.${name}`);
    }
    ...
    for (const [name,keys] of Object.entries(spec.keys)) {
      ...
      if (actual.map((row)=>row.column_name).join(',')!==keys.join(',') || (name==='PRIMARY' && actual.some((row)=>Number(row.non_unique)!==0))) throw new Error(`B9 index conflict: ${table}.${name}`);
    }
...
async function runMigration(conn,{ apply=false }={}) {
  ...
  const verify=(state)=> {
    if (state.checksum && state.checksum!==migration.checksum) throw new Error('B9 checksum mismatch');
    if (state.checksum && state.plan.some((item)=>item.status!=='applied')) throw new Error('B9 schema is incomplete');
  };
  if (!apply) { const state=await readSchema(conn); verify(state); return state.plan; }
  const [lock]=await conn.execute('SELECT GET_LOCK(?,10) AS acquired',['qydailian_b9_order_center']);
  if (Number(lock[0]?.acquired)!==1) throw new Error('Cannot acquire B9 migration lock');
  try {
    ...
    if (after.plan.some((item)=>item.status!=='applied')) throw new Error('B9 verification failed');
    if (!before.checksum) await conn.execute('INSERT INTO schema_migrations (version,checksum) VALUES (?,?)',[VERSION,migration.checksum]);
    return after.plan;
  } finally { await conn.execute('SELECT RELEASE_LOCK(?)',['qydailian_b9_order_center']); }
}
