...
const VERSION = '20260916_b11_order_cleanup';
const SPECS = {
  order_removals: { columns: {order_type:'varchar(20)',order_ref:'varchar(64)',removed_at:'datetime',removed_by:'int',reason:'varchar(500)',state_snapshot:'varchar(40)',payment_snapshot:'varchar(30)'},keys:{PRIMARY:['order_type','order_ref']} },
  order_cleanup_settings: { columns: {id:'int',enabled:'tinyint unsigned',retention_days:'int unsigned',updated_by:'int'},keys:{PRIMARY:['id']} }
};
function loadMigration() {
  ...
  if (tables.join(',') !== Object.keys(SPECS).join(',')) throw new Error('B11 permits only the two reviewed additive tables');
...
async function readSchema(conn) {
  ...
  for (const [table,spec] of Object.entries(SPECS)) {
    ...
    for (const [name,type] of Object.entries(spec.columns)) {
      if (!columns.some((row)=>row.name===name && row.type.toLowerCase()===type)) throw new Error(`B11 schema conflict: ${table}.${name}`);
    }
    ...
    for (const [name,keys] of Object.entries(spec.keys)) {
      ...
      if (actual.map((row)=>row.column_name).join(',')!==keys.join(',') || (name==='PRIMARY' && actual.some((row)=>Number(row.non_unique)!==0))) throw new Error(`B11 index conflict: ${table}.${name}`);
    }
...
async function runMigration(conn,{ apply=false }={}) {
  ...
  const verify=(state)=> {
    if (state.checksum && state.checksum!==migration.checksum) throw new Error('B11 checksum mismatch');
    if (state.checksum && state.plan.some((item)=>item.status!=='applied')) throw new Error('B11 schema is incomplete');
  };
  if (!apply) { const state=await readSchema(conn); verify(state); return state.plan; }
  const [lock]=await conn.execute('SELECT GET_LOCK(?,10) AS acquired',['qydailian_b11_order_cleanup']);
  if (Number(lock[0]?.acquired)!==1) throw new Error('Cannot acquire B11 migration lock');
  try {
    ...
    if (after.plan.some((item)=>item.status!=='applied')) throw new Error('B11 verification failed');
    if (!before.checksum) await conn.execute('INSERT INTO schema_migrations (version,checksum) VALUES (?,?)',[VERSION,migration.checksum]);
    return after.plan;
  } finally { await conn.execute('SELECT RELEASE_LOCK(?)',['qydailian_b11_order_cleanup']); }
}
