(function (global) {
  ...
  const actionNames = {boost_review:'审核结单截图',pay:'继续支付',refresh:'刷新到账状态',provider:'查询支付宝',reconcile:'重试到账',boost_payment:'上传付款凭证',boost_confirm_payment:'核实收款',boost_dispatch:'放入接单大厅',rental_manage:'处理租号订单',third_party_manage:'处理三方订单',archive:'归档',unarchive:'恢复归档'};
  ...
  async function perform(action) {
    ...
    if(action==='boost_review'){close();global.BoostCompletion.loadAdmin();document.getElementById('boostCompletionQueue')?.scrollIntoView({behavior:'smooth',block:'start'});return;}
