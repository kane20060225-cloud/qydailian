(function (global) {
  ...
  function setupPanel(scope) {
    ...
    const p=panels[scope]={root,list:$(admin?'adminOrderList':'orderList'),generation:0,rows:[],filters:{type:'',state:admin?'todo':'',task:'',search:'',from:'',to:'',channel:'',archived:'0',page:1}};
    ...
    root.addEventListener('click',async e=>{
      ...
      else if(b.hasAttribute('data-reset')) {p.filters={type:'',state:admin?'todo':'',task:'',search:'',from:'',to:'',channel:'',archived:'0',page:1};root.querySelector('form').reset();statusOptions(scope);load(scope);}
  ...
  async function showDetail(order,scope='user') {
    ...
    try{const data=await request(`/order-center/${order.order_type}/${encodeURIComponent(order.order_ref)}?${new URLSearchParams(scope==='admin'?{scope:'admin'}:{})}`);if(generation!==detailGeneration)return;detail=data.order;
      const pairs=[{label:'订单状态',value:detail.state_label},{label:'订单金额',value:amount(detail)},{label:'订单内容',value:detail.title},{label:'创建时间',value:time(detail.created_at)},{label:'支付渠道',value:detail.payment_channel},{label:'付款用户',value:detail.customer_name},...data.details];
  ...
  async function perform(action) {
    ...
    try {
      if(action==='pay'){
        ...
        if(['reconcile','boost_confirm_payment','archive','unarchive'].includes(action)&&!reason)throw new Error('请填写操作说明');
  ...
  function init(options) {
    ...
    modal.addEventListener('click',e=>{if(e.target===modal||e.target.closest('#ocClose')){if(!busy)close();return;}if(e.target.id==='ocDetailRetry')showDetail(detail,detailScope);const b=e.target.closest('button');if(!b)return;if(b.dataset.action){if(['reconcile','boost_confirm_payment','boost_dispatch','archive','unarchive'].includes(b.dataset.action))reasonForm(b.dataset.action);else perform(b.dataset.action);}if(b.dataset.confirm)perform(b.dataset.confirm);if(b.hasAttribute('data-cancel-reason'))$('ocActionForm').innerHTML='';});
