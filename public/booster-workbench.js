(function(global){
 ...
 function cards(rows,isHall){
  ...
  return `<div class="bw-order-grid">${rows.map(o=>{
   ...
   return `<article class="card bw-order-card"><div class="bw-order-top"><span class="bw-order-ref">${esc(o.order_no)}</span><span class="bw-status ${o.status==='done'?'bw-done':''}">${isHall?'待接单':o.completion_status==='pending'?'待审核结单':({playing:'代练中',done:'已完成',pending:'待处理',cancelled:'已取消'})[o.status]||esc(o.status)}</span></div>
    ...
    ${o.completion_status==='rejected'?`<p class="bw-legacy-note">审核未通过：${esc(o.completion_reason||'请补充有效结单截图')}</p>`:''}<dl class="bw-order-meta"><div><dt>服务数量</dt><dd>${esc(o.quantity)}</dd></div><div><dt>下单时间</dt><dd>${time(o.created_at)}</dd></div></dl>
    <div class="bw-order-footer"><div><span>${o.test_order?'历史测试订单 · 不计收益':o.earnings_reversed?'测试收益已冲正':settled?'已入账收益':o.status==='done'?'历史预估 · 未关联流水':'预估收益'}</span><strong>¥${money(o.test_order||o.earnings_reversed?0:settled?o.settled_earnings:o.earnings)}</strong></div><div class="bw-order-actions">${isHall?`<button type="button" class="take-order-btn bw-primary" data-order="${esc(o.order_no)}">接单</button>`:`${o.status==='playing'&&o.completion_status!=='pending'?`<button type="button" class="complete-order-btn bw-primary" data-order="${esc(o.order_no)}">${o.completion_status==='rejected'?'补充截图':'提交结单'}</button>`:''}<button type="button" class="detail-btn" data-order="${esc(o.order_no)}">详情</button>`}</div></div>
   </article>`;
  }).join('')}</div>`;
 }
 function renderHall(){
 ...
 function renderMy(){
  const root=el('myBoosterOrderList'),rows=mine.filter(o=>state.myStatus==='all'||(state.myStatus==='review'?o.completion_status==='pending':o.status===state.myStatus&&(state.myStatus!=='playing'||o.completion_status!=='pending')));
  root.querySelector('[data-list]').innerHTML=cards(rows,false);
  root.querySelectorAll('[data-my-status]').forEach(b=>{b.classList.toggle('active',b.dataset.myStatus===state.myStatus);b.setAttribute('aria-pressed',String(b.dataset.myStatus===state.myStatus));b.querySelector('span').textContent=mine.filter(o=>b.dataset.myStatus==='all'||(b.dataset.myStatus==='review'?o.completion_status==='pending':o.status===b.dataset.myStatus&&(b.dataset.myStatus!=='playing'||o.completion_status!=='pending'))).length;});
 }
 async function loadHall(){
  const root=el('hallOrderList');
  if(!root.querySelector('[data-list]'))root.innerHTML=`<div class="bw-page-heading"><div><span class="bw-eyebrow">ORDER HALL</span><h3>找到适合你的订单</h3><p>按身份要求展示可接订单，加急优先、同类订单按等待时间排列。</p></div><button type="button" data-hall-refresh>刷新大厅</button></div><div class="bw-toolbar"><label>查找订单<input name="search" type="search" placeholder="订单号、项目或方案"></label><label>客户端<select name="client"><option value="">全部客户端</option><option>Android</option><option>iOS</option></select></label><span data-count></span></div><p class="bw-note">当前分成：现金结算价的75%。预估收益尚未入账，截图经管理员审核通过后结算。</p><div data-list>${empty('正在加载可接订单…')}</div>`;
  try {
 ...
 async function loadMy(){
  ...
  if(!root.querySelector('[data-list]'))root.innerHTML=`<div class="bw-page-heading"><div><span class="bw-eyebrow">MY ORDERS</span><h3>专注正在处理的订单</h3><p>完成服务后上传结单截图，管理员审核通过后才计入收益。</p></div><button type="button" data-my-refresh>刷新订单</button></div><div class="bw-periods" aria-label="我的接单状态">${[['playing','进行中'],['review','待审核'],['done','已完成'],['all','全部']].map(([key,label])=>`<button type="button" data-my-status="${key}">${label} <span>0</span></button>`).join('')}</div><div data-list>${empty('正在加载订单…')}</div>`;
 ...
 function renderFinance(data){
  ...
  root.querySelector('[data-finance-content]').innerHTML=`<div class="bw-summary-grid">${summary.map(([title,amount,note],i)=>`<section class="card bw-summary-card ${i===0?'bw-summary-primary':''}"><h4>${title}</h4><strong>¥${money(amount)}</strong><p>${note}</p></section>`).join('')}</div>
   ...
   <section class="card bw-ledger-card"><div class="bw-card-heading"><h4>收益明细</h4><span>共 ${esc(s.total)} 条记录</span></div><p class="bw-note">正数为入账，负数为冲正／调整；原收入与冲正记录均保留；历史测试收入已清零，旧流水可在订单中心核对。</p>
